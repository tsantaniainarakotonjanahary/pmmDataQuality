...
{
    const response = await fetch("https://ministere-sante.mg/api/29/analytics.json?dimension=pe:LAST_MONTH;LAST_3_MONTHS&dimension=dx:JTfvT0DrBvz;Nm1qAOpMbDX;ip8qeFyILRB;sYVDf23mZsg;d7jhJrTl6ig;LdkpYulBjdW;HGALcAqspdR;ebV6IUA6Xwk;GkEv4YDqZ40&filter=ou:"+req.query.ou+"&displayProperty=NAME&outputIdScheme=NAME",{
...
{
    const response = await fetch("https://covax.vaksiny.gov.mg/api/29/analytics.json?dimension=dx:TPW8h9OYSkS;aSDvXnavplO;ziprLDlyUDf;ulBHnoeCEkm;zqTS3RiSL3o;K4UPUEDKFZJ;RkC5nQeFp1f;Kkbt3LgbCbD&dimension=pe:LAST_MONTH;LAST_3_MONTHS&filter=ou:"+req.query.ou+"&displayProperty=NAME&outputIdScheme=NAME",{
        headers: { Authorization: `Basic ${Buffer.from( "Nosybe" + ":" + "2021@Covax" ).toString("base64")}`, },
      }
    );
    var s = await response.json();
...
{
    const response = await fetch("https://covax.vaksiny.gov.mg/api/29/analytics.json?dimension=pe:LAST_MONTH;LAST_3_MONTHS&dimension=dx:LWmmFCXgUmg;hJKdezT5GcK;f6JAIJEpHpe;OJUzxCAcudW;D20DogInJTS&filter=ou:"+req.query.ou+"&displayProperty=NAME&outputIdScheme=NAME",{
        headers: { Authorization: `Basic ${Buffer.from( "Scorcard" + ":" + "D@shb0ard" ).toString("base64")}`, },
      }
    );
    var s = await response.json();
