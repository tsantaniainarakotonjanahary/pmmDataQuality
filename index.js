  ...
  if (status == "200") {
    ...
    for (var i = 0; i < height; i++) {
      if (
        ...
        if (data.rows[i][13].replace(/\s/g, "").trim() === "1") {
          data.rows[i][13] = "Agent de santé";
        } else if (data.rows[i][13].replace(/\s/g, "").trim() === "2") {
          data.rows[i][13] = "Force de l'ordre";
        } else if (data.rows[i][13].replace(/\s/g, "").trim() === "3") {
          data.rows[i][13] = "Personne âgée";
        } else if (data.rows[i][13].replace(/\s/g, "").trim() === "4") {
          data.rows[i][13] = "Travailleurs sociaux";
        } else if (data.rows[i][13].replace(/\s/g, "").trim() === "5") {
